...
describe("EthanToken", function () {
  it("Test contract", async function () {
    ...
    it("Test mint", async function () {
      ...
      const instance = await ContractFactory.deploy();
      await instance.deployed();
      await instance.mint(addr1.address, ethers.utils.parseUnits("999999999", "ether"));

      expect(await instance.balanceOf(addr1.address)).to.equal(ethers.utils.parseUnits("999999999", "ether"));
    });
    ...
    it("Test mint reach hard limit", async function () {
      const [owner, addr1, addr2] = await ethers.getSigners();
      const ContractFactory = await ethers.getContractFactory("EthanToken");

      const instance = await ContractFactory.deploy();
      await instance.deployed();
      await expect(instance.mint(addr1.address, ethers.utils.parseUnits("1000000001", "ether"))).to.be.revertedWith('Not possible to mint tokens more than 1b hard limit');
    });
